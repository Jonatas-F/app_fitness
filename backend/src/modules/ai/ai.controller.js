...
export async function handleGenerateAiWorkout(req, res, next) {
  try {
    const { goal, persist, trainingAvailableDays, trainingExperience, trainingAge, availableMinutes, trainingPreference, trainingPreferenceFreeText, muscleGroupCombinations, workoutDayProtocol, favoriteExercises, trainingFocus, adherenceAdjustedDays, keepWorkoutProtocol, lastProtocolFeeling, muscularSoreness, generalDisposition, laggingMuscleGroups, requestedWorkoutChanges } = req.body;
    const result = await generateAiWorkoutPlan(req.auth.sub, { goal, persist, trainingAvailableDays, trainingExperience, trainingAge, availableMinutes, trainingPreference, trainingPreferenceFreeText, muscleGroupCombinations, workoutDayProtocol, favoriteExercises, trainingFocus, adherenceAdjustedDays, keepWorkoutProtocol, lastProtocolFeeling, muscularSoreness, generalDisposition, laggingMuscleGroups, requestedWorkoutChanges });
    res.json(result);
  } catch (error) {
    next(error);
  }
